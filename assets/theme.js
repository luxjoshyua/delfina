...
//   	  Instagram
      ...
      getImages = function() {
        return $.ajax({
          dataType: "jsonp",
          url: 'https://api.instagram.com/v1/users/self/media/recent/?access_token=1082928977.1677ed0.a01985b8326d4e6a9fe8afc6afdc51d0' + '&count=6',
            ...
            if (response.meta.code === 200) {
              ...
              for (i = _i = 0, _ref = items_to_load - 1; 0 <= _ref ? _i <= _ref : _i >= _ref; i = 0 <= _ref ? ++_i : --_i) {
                if (data[i]) {
                  ...
                  _results.push($('.instagram-block').append('<a class="grid__item medium-up--one-third small--one-half" target="_blank" href="' + data[i].link + '"><div class="instagram-box" style="background-image: url(' + img_src + ')"><div class="instagram-overlay">' + instagramIcon + '</div></div></a>'));
                } else {
  ...
  $('.template-index .quote__row').slick({
    slidesToShow: 3,
    slidesToScroll: 1,
    autoplay: true,
    autoplaySpeed: 1500,
    dots: false,
    prevArrow: '<button type="button" class="slick-prev"><img src="https://cdn.shopify.com/s/files/1/0073/7234/1346/t/4/assets/chevron_left.png?10628277705118452880">',
    nextArrow: '<button type="button" class="slick-next"><img src="https://cdn.shopify.com/s/files/1/0073/7234/1346/t/4/assets/chevron_right.png?6682342823898560379">',
    arrows: true,
    responsive: [
      {
        breakpoint: 900,
  ...
  $('.tabs-block').on('click', function(){
        $('.tabs-block').not(this).removeClass('tabs-block-active');
        $(this).toggleClass('tabs-block-active');  
  });
  ...
  $(".image-upload").change(function(){
    ...
    $(this).next().find('span').text('File uploaded');
  });
  ...
  // Remove popup if click outside of the popup

  $(document).click(function(event) {
    //if you click on anything except the modal itself or the "open modal" link, close the modal
    if (!$(event.target).closest(".overlay > div,.popup-trigger").length) {
  ...
  $('.quick-order__thumbnail-item a').on('click', function(e){
  	e.preventDefault();
  	var imgUrl = $(this).attr('href');
    $('.quick-order__featured-image img').attr('src', imgUrl);
  });

   $('.quick-order__form .selector-wrapper select').on('change',function() {
    var selectedVariant = $(this).val().trim();
    $(this).closest('form').find('.product-form__variants option').each(function() {
        var variantName = $(this).text().trim();
        if(variantName == selectedVariant) {
          $(this).closest('form').find('.btn').prop("disabled", false).text('Add to cart');
          $(this).prop("selected", true);
          return false;
        } else {
          $(this).closest('form').find('.btn').prop("disabled", true).text('Sold out');
        }
    })
  });
  ...
  $('.popup').on('change','.get-started__card input', function(){
    var product = $(this);
    var container = product.closest('.get-started__group');
    var productId = product.closest('.get-started__card').data('id');
    var productUrl = product.closest('.get-started__card').data('url');
    var productImage = product.closest('.get-started__card').data('image');
    var producttitle = product.closest('.get-started__card').data('title');
    
    var productRow = '<li id="' + productId;
        productRow = productRow + '" class="get-started__selection-item">';
    	productRow = productRow + '<div class="get-started__selection-image"><img src="';
        productRow = productRow + productImage;
    	productRow = productRow + '"></div><div class="get-started__selection-name"><p>';
        productRow = productRow + producttitle;
        productRow = productRow + '</p></div><div class="get-started__selection-view"><p><a href="';
        productRow = productRow + productUrl;
        productRow = productRow + '" target="_blank">View</a></p></div><div class="get-started__selection-remove"><p><a href="#">Remove</a></p></li>';

    
        if (product.is(':checked')) { 
          container.find('.get-started__selection-box--list').append(productRow);
          productCount(container);  
        } else {
          $('.get-started__selection-box--list').find('#' + productId).remove();
          productCount(container);
          
        }
  });
  
  // Get started remove button
  $('.get-started__selection-box--list').on('click', '.get-started__selection-remove', function(e){
    e.preventDefault();
  	var elementId = $(this).closest('.get-started__selection-item').attr('id');
    var container = $(this).closest('.get-started__group');
    
    $('#popup-' + elementId).find('input').prop('checked',false);
    
    $(this).closest('.get-started__selection-item').remove();
    
    productCount(container);
  });
  
  $('.suffix-get-started form').on('submit', function(){
    console.log('submit');
  	$(this).find("input[type='submit']").addClass('btn-uploading');
  });
  
  $(".get-started__section input[type='file']").on('change', function() {
      $(this).next().addClass('btn-uploading');
  });
  
  function productCount(container, productsCount) {
        var count = container.find('.get-started__selection-box--list li').length;
  		if(count > 0) {
          container.find('.get-started__status-bar.empty').hide();
          container.find('.get-started__status-bar.selected').show(); 
          container.find('.empty-trigger').hide();
          container.find('.full-trigger').show();
        } else {
          container.find('.get-started__status-bar.empty').show();
          container.find('.get-started__status-bar.selected').hide(); 
          container.find('.empty-trigger').show();
          container.find('.full-trigger').hide();
        }
  };
 
    function addAllGallery(collection, counter) {
      $.ajax({
        type: 'GET',
        url: '' + collection + '?page=' + counter + '',
        beforeSend: function() {
        ...
        success: function(data) {
          ...
          var filteredData = $(data).find(".grid--view-items .grid__item");

          $(".gallery-popup-products .grid").append( filteredData ); 

          if(filteredData.length < 2) {
            return
          }   
          
          addAllGallery(collection, counter);
        },
        error: function(data) {
          return
        },
  ...
  $('.gallery-collection-trigger').on('click', function(e){
  	e.preventDefault();
    var collectionURL = $(this).attr('href');       
    addAllGallery(collectionURL, 1);
    
    $('.gallery-popup-collections').removeClass('is-visible');
    $('.gallery-popup-products').addClass('is-visible');
  });
  
  $('.go-back').on('click', function(e){
  	e.preventDefault();
    $(".gallery-popup-products .grid").empty();
    
    $('.gallery-popup-collections').addClass('is-visible');
    $('.gallery-popup-products').removeClass('is-visible');
  });
  
  
  // Infinite scrolling on Get started page
//   var counter = 2;
//   function ScrollExecute() {
//       scrollNode = $('#more').last();  
//       scrollURL = $('#more').attr("href");  
//       console.log(scrollURL);
//         $.ajax({
//           type: 'GET',
//           url: '/collections/gallery?page=2',
//           beforeSend: function() {
...
//             counter = counter + 1;
//             console.log(counter);
//             $('#more').last().attr("href" , "" + window.location.href + "?page=" + counter + "" );
//             var filteredData = $(data).find(".grid--view-items").addClass('main-image-loaded');
//             $("body").append( filteredData );    
//             if(filteredData.length < 2) {
//                 scrollNode.hide();
//             }              
//           },
